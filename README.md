...
3. Or we can go to [Chainlist](https://chainlist.org/) and copy the RPCs are available
4. Grab your URL associated with the testnet, and place it into your `.env` file.
5. Make sure you have [testnet ETH](https://faucets.chain.link/)/[testnet FTM](https://faucet.fantom.network/) in your account. You can [get some here](https://faucets.chain.link/). You should get testnet ETH for the same testnet that you made a project in Alchemy (ie, Goerli)
6. Run

```
forge create --private-key <PRIVATE_KEY> --rpc-url <ALCHEMY_URL>/<RPC_URL>
```

Or, you can use a deploy script!

```
forge script script/DeploySimpleStorage.s.sol --private-key <PRIVATE_KEY> --rpc-url <ALCHEMY_URL>/<RPC_URL>
```
